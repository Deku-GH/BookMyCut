...
<body>
    ...
    <aside class="sidebar" id="sb">
        ...
        <nav class="sb-nav">
            ...
            <a class="ni" href="{{ route('timeslots.index') }}"><i class="bi bi-clock" ></i><span class="lbl">timeslots</span></a>
    ...
    <div class="main" id="mn">
        ...
        <div class="topbar">
            <div>
                <div class="t-title">Mes Services</div>
                <div class="t-sub">Gérez votre catalogue de services</div>
            </div>
            <div class="d-flex align-items-center gap-2">
                <button class="btn-g" data-bs-toggle="modal" data-bs-target="#createModal">
        ...
        <di class="content">
            ...
            @endif
            @if($errors->any())
                <div class="alert-err-custom mb-4">
            ...
            <div class="row g-3 mb-4">
                <div class="col-6 col-xl-3">
                    <div class="sc">
                        ...
                        <div class="sc-ico g"><i class="bi bi-list-stars"></i></div>
                        <div class="sc-val">{{ $services->count() }}</div>
                        <div class="sc-lbl">Services total</div>
                    </div>
                </div>
                <div class="col-6 col-xl-3">
                    <div class="sc">
                        <div class="sc-bar" style="background:var(--green)"></div>
                        <div class="sc-ico gr"><i class="bi bi-check-circle"></i></div>
                        <div class="sc-val">{{ $services->where('active', true)->count() }}</div>
                        <div class="sc-lbl">Actifs</div>
                    </div>
                </div>
                <div class="col-6 col-xl-3">
                    <div class="sc">
                ...
                <div class="col-6 col-xl-3">
                    <div class="sc">
                        ...
                        <div class="sc-val">{{ $services->avg('prix') ? number_format($services->avg('prix'), 0) : '—' }}
                        </div>
                        <div class="sc-lbl">Prix moyen (MAD)</div>
                    </div>
                </div>
            </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                    <div class="tc">
                        <div class="th">
                            <h6><i class="bi bi-list-stars me-2" style="color:var(--gold)"></i>Catalogue des services
                            </h6>
                            <div class="d-flex gap-2 align-items-center">
                                {{-- Search filter (frontend only) --}}
                                <input type="text" id="searchInput" class="form-control form-control-sm"
                                    placeholder="Rechercher..." style="width:160px" oninput="filterTable()" />
                                <button class="btn-g" data-bs-toggle="modal" data-bs-target="#createModal">
                        ...
                        <div class="table-responsive">
                            <table class="tt" id="servicesTable">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                ...
                                <tbody>
                                    @forelse($services as $index => $service)
                                        <tr data-name="{{ strtolower($service->titre) }}">
                                            ...
                                            <td>
                                                <div style="display:flex;align-items:center;gap:0.6rem;">
                                                    <div
                                                        style="width:32px;height:32px;background:var(--gold-dim);border-radius:7px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                                        <i class="bi bi-scissors"
                                                            style="color:var(--gold);font-size:0.85rem"></i>
                                                    </div>
                                                    <div>
                                                        <div style="color:var(--white);font-weight:600;font-size:0.86rem">
                                                            {{ $service->titre }}</div>
                                                        <div style="color:var(--muted);font-size:0.74rem">
                                                            {{ Str::limit($service->description ?? '', 35) }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                @if($service->category)
                                                    <span class="pill pb">{{ $service->category->name }}</span>
                                                @else
                                            ...
                                            <td>
                                                <span style="display:flex;align-items:center;gap:0.3rem">
                                                    <i class="bi bi-clock" style="color:var(--muted);font-size:0.82rem"></i>
                                                    {{ $service->duration }} min
                                                </span>
                                            </td>
                                            <td style="color:var(--gold);font-weight:600">{{ $service->prix }} MAD</td>
                                            <td>
                                                <span class="pill {{ ($service->active ?? true) ? 'pg' : 'pr' }}">
                                                    {{ ($service->active ?? true) ? 'Actif' : 'Inactif' }}
                                                </span>
                                            </td>
                                            <td>
                                                <div style="display:flex;gap:0.35rem">Catégories
                                                    {{-- EDIT BUTTON --}}
                                                    <a class="btn-sm eg"
                                                    ...
                                                    <form action="{{ route('services.destroy', $service->id) }}"
                                                        ...
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn-sm er"
                                    ...
                                    @empty
                                        <tr class="empty-row">
                                            <td colspan="7">
                                                <div
                                                    style="display:flex;flex-direction:column;align-items:center;gap:0.8rem;padding:2.5rem 0">
                                                    <i class="bi bi-inbox" style="font-size:2.5rem;color:var(--muted)"></i>
                                                    <span>Aucun service trouvé. Créez votre premier service.</span>
                                                    <button class="btn-g" data-bs-toggle="modal"
                        ...
                        @if($services instanceof \Illuminate\Pagination\LengthAwarePaginator)
                            <div class="p-3 d-flex justify-content-between align-items-center"
                                ...
                                <span style="font-size:0.79rem;color:var(--muted)">
                                    {{ $services->firstItem() }}–{{ $services->lastItem() }} sur {{ $services->total() }}
                                    services
                                </span>
            ...
            <form method="POST" action="{{ route('create.service') }}" class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="createModalLabel">
                        <i class="bi bi-plus-circle me-2" style="color:var(--gold)"></i>Nouveau service
                    </h5>
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="titre" class="form-control" placeholder="Ex: Coupe dégradée"
                                required value="{{ old('titre') }}" />
                        </div>
                        <div class="col-sm-6">
                            <label class="form-label">Durée (min) <span style="color:var(--red)">*</span></label>
                            <input type="number" name="duration" class="form-control" value="{{ old('duration', 30) }}"
                                min="5" max="300" required />
                        </div>
                        <div class="col-sm-6">
                            <label class="form-label">Prix (MAD) <span style="color:var(--red)">*</span></label>
                            <input type="number" name="prix" class="form-control" value="{{ old('prix', 50) }}" min="0"
                                step="0.01" required />
                        </div>
                        <div class="col-12">
                            <label class="form-label">Description</label>
                            <textarea class="form-control" name="description"
                                placeholder="Décrivez ce service...">{{ old('description') }}</textarea>
                        </div>
                        <div class="col-12">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn-g">
                        <i class="bi bi-check2-circle"></i>Créer le service
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <form method="POST">
                ...
                <div class="modal-body p-4 text-center">
                    <div
                        style="width:52px;height:52px;background:rgba(239,68,68,0.1);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 1rem">
                        <i class="bi bi-exclamation-triangle-fill" style="color:var(--red);font-size:1.4rem"></i>
                    </div>
                    <div
                        style="font-family:'Playfair Display',serif;font-size:1rem;color:var(--white);margin-bottom:0.5rem">
                        Confirmer la suppression</div>
                    <div style="font-size:0.84rem;color:var(--muted)">
                        Voulez-vous supprimer <strong id="delete_name" style="color:var(--white)"></strong> ? Cette
                        action est irréversible.
                    </div>
                </div>
                <div class="modal-footer justify-content-center gap-2">
                    <button type="button" class="btn-do" data-bs-dismiss="modal">Annuler</button>
                    <button type="submit" class="btn-sm er" style="padding:0.5rem 1.1rem;font-size:0.83rem">
